...
アイコンの原本は `src-tauri/icons/` の SVG である。
形と色の決まりは [DESIGN.md](DESIGN.md) 第 10 節にある。

SVG を直したら、リポジトリの根で書き出しを走らせ、生成物もあわせてコミットする。
...
ICO の先頭は 32px にしてある。
Tauri はウィンドウの既定のアイコンに先頭の 1 枚を使うため、16px を先頭にするとタスクバーでぼやける。

タイトルバーの 16px のアイコンは `source-16.svg` を画面が直接読むので、書き出しは要らない。

## 3. テスト

### 3.1 現状

src-tauri で 169 個、移行ツールで 3 個、合わせて 172 個。
...
- [ ] タスクバーだけを明るく（または暗く）しても、トレイのアイコンが見える色に切り替わる
- [ ] 表示倍率 100%、150% で、トレイのアイコンがぼやけない
- [ ] Windows 11 で Mica が適用され、ウィンドウの背景に材質が見える（壁紙を変えると色味が変わる）
- [ ] Mica が使えない環境（Windows 10 など）では、不透明な背景で表示され、下が透けない
- [ ] 100% 以外の表示倍率でも配置が崩れない
- [ ] `Tab` でボタンと棚の行を移ると、フォーカスの輪郭線が出る
- [ ] 棚が 1 つも無いとき、空の棚、検索で 0 件のときに、一覧の中に案内が出る
- [ ] エクスプローラからドラッグしている間、追加先の棚の名前と件数が出て、離すと消える
- [ ] 取り込みのダイアログで、やめる、`Escape`、背景のクリックのどれでも何も取り込まれない
